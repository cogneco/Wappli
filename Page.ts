...
module Wappli {
	export class Page {
		...
		constructor(private name: string, private icon: string = null, private dialog: boolean = false) {
			...
			// Back Button
			if (!this.dialog) {
				this.backButton = <HTMLAnchorElement>document.createElement("a");
		...
		Initialize(): boolean {
			...
			var me = this;
			$(this.page).bind("pageshow", (event: any) => {
				if (me.invalidated)
					me.Update();
			});
			this.Setup();
		...
		private CreateChildLink(child: Page): HTMLAnchorElement {
			...
			var result: HTMLAnchorElement = <HTMLAnchorElement>document.createElement("a");
